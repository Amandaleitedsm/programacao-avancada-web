...
require_once 'api_2bim/src/DAO/alunosDAO.php';
    class AlunosMiddleware 
    {
        public function stringJsonToStdClass($requestBody): stdClass{
            ...
            else if (!isset($stdAluno->alunos->dataNascimento)){
                (new Response(
                    success: false,
                    message: "Aluno inválido",
                    error:[
                        "code" => 'validation_error',
                        "message" => 'Não foi enviado a data de nascimento de um aluno.'
                    ],
                    httpCode: 400
                ))->send();
                exit();
            }
            else if (!isset($stdAluno->alunos->idTurma)){
                (new Response(
                    success: false,
                    message: "Aluno inválido",
                    error:[
                        "code" => 'validation_error',
                        "message" => 'Não foi enviado a turma de um aluno.'
                    ],
                    httpCode: 400
                ))->send();
                exit();
